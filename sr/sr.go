...
type SRH interface {
	...
	OnSrF(hs *routing.HTTPSession, sp, sf string) error
}
type SR struct {
	H SRH
...
func (s *SRH_N) OnSrF(hs *routing.HTTPSession, sp, sf string) error {
	return nil
}
...
type SRH_Q_H interface {
	Args(s *SRH_Q, hs *routing.HTTPSession, sp, sf string) (util.Map, error)
...
func (s *SRH_Q) OnSrF(hs *routing.HTTPSession, sp, sf string) error {
	if !s.Running {
		log.W("SRH_Q OnSrF err:Proc is not running")
		return util.Err("SRH_Q not running")
	}
	kvs, err := s.H.Args(s, hs, sp, sf)
	if err == nil {
		s.Q <- &SRH_Q_I{
			Sp:  sp,
			Kvs: kvs,
